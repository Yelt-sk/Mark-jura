 ...
 */
export function Canvas({ steps, moduleMap, onSettingsChange, onRemoveStep }: CanvasProps) {
  const { isOver, setNodeRef } = useDroppable({ id: 'canvas-drop-zone' });

  return (
    <section className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
      <header className="mb-4">
        <h2 className="text-lg font-semibold text-slate-800">Конструктор стратегии</h2>
      ...
      <div
        ...
        className={`min-h-[440px] rounded-xl border border-dashed p-3 transition ${
          isOver ? 'border-blue-400 bg-blue-50/50' : 'border-slate-300 bg-slate-50/60'
        }`}
      >
        {steps.length === 0 ? (
          <motion.div
            ...
            className="flex h-[400px] items-center justify-center rounded-lg border border-slate-200 bg-white/80 text-center"
          >
